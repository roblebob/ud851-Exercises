...
public class ReminderUtilities {
    ...
    // _TODO (16) Create a synchronized, public static method called scheduleChargingReminder that takes
    ...
    synchronized public static void scheduleChargingReminder( Context context) {
        // _TODO (17) If the job has already been initialized, return
        if (sInitialized) return;
        // _TODO (18) Create a new GooglePlayDriver
        GooglePlayDriver googlePlayDriver = new GooglePlayDriver( context);
        // _TODO (19) Create a new FirebaseJobDispatcher with the driver
        FirebaseJobDispatcher firebaseJobDispatcher = new FirebaseJobDispatcher( googlePlayDriver);
        // _TODO (20) Use FirebaseJobDispatcher's newJobBuilder method to build a job which:
        Job constraintReminderJob = firebaseJobDispatcher .newJobBuilder()
                ...
                // - has the job recur
                .setRecurring( true)
                // - occurs every 15 minutes with a window of 15 minutes. You can do this using a setTrigger, passing in a Trigger.executionWindow
        ...
        // _TODO (21) Use dispatcher's schedule method to schedule the job
        firebaseJobDispatcher.schedule( constraintReminderJob);

        // _TODO (22) Set sInitialized to true to mark that we're done setting up the job
        sInitialized = true;
    }
}